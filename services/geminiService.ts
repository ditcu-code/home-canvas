    ...
    return new Promise((resolve, reject) => {
        ...
        reader.onload = (event) => {
            ...
            img.onload = () => {
                ...
                const hex = `#${[rAvg, gAvg, bAvg].map(v => Math.round(v).toString(16).padStart(2, '0')).join('')}`;
                ...
                const sharpLabel = sharpness < 0.05 ? 'soft/blurred (shallow DOF)'
                                   : sharpness < 0.12 ? 'moderately sharp'
                                   : 'very sharp';
                ...
                const description = `Local light near placement is ${tempLabel} (avg ${hex}, hue ${Math.round(hue)}°, sat ${satPct}%, lightness ${lightPct}%). The area appears ${sharpLabel}.`;
                resolve(description);
            };
...
export const generateCompositeImage = async (
  ...
  const localLightingHint = await analyzeLocalLighting(resizedEnvironmentImage, dropPosition, { originalWidth, originalHeight });
